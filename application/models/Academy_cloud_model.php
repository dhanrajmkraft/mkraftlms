...
class Academy_cloud_model extends CI_Model
{
    ...
    function upload_video(){
      ...
      curl_setopt_array($curl, array(
        ...
        CURLOPT_HTTPHEADER => array(
          'Authorization: '.get_settings('mkraft_cloud_access_token'),
    ...
    function save_access_token(){
      ...
      $query = $this->db->get_where('settings', ['key' => 'mkraft_cloud_access_token']);
      if($query->num_rows() > 0){
        $this->db->where('key', 'mkraft_cloud_access_token');
        $this->db->update('settings', $data);
      }else{
        $data['key'] = 'mkraft_cloud_access_token';
        $this->db->update('settings', $data);
      }
    }
